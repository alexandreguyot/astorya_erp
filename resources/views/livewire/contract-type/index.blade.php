    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="ContractType" format="csv" />
                <livewire:excel-export model="ContractType" format="xlsx" />
                <livewire:excel-export model="ContractType" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($contractTypes as $contractType)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $contractType->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $contractType->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('contract_type_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.contract-types.show', $contractType) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
